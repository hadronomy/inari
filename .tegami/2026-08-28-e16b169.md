...
The Overview gate's connectors are live wires now. While traffic passes, a
crest of light travels the wire from this computer, through the agent, out to
the devices — the same staggered phase wave the alert's cascade uses, so
"live" reads as motion in the app's one visual language. A broken path goes
still: the wire holds its gap, and the cross settles in so the failure
arrives rather than pops. With motion off, the wire reads as the same path at
rest.
