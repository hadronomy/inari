...
class DriverRegistry:
    ...
    def drivers_for(self, kind: DeviceKind, *, available_only: bool = False) -> tuple[DeviceDriver, ...]:
        drivers = tuple(self._drivers.get(kind, ()))
        if not available_only:
            return drivers
        return tuple(driver for driver in drivers if driver.is_available())

    def printer_drivers(self, *, available_only: bool = False) -> tuple[PrinterDriver, ...]:
