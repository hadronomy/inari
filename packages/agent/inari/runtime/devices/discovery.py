...
class DiscoveryCoordinator:
    ...
    async def sync_once(self) -> tuple[DeviceRecord, ...]:
        ...
        current = {
            device.id: device
            for device in self._discover_devices(observed_at=observed_at)
        }
        ...
        for device_id, prior in previous.items():
            if (
                device_id in current
                or prior.connection_state is DeviceConnectionState.OFFLINE
            ):
                continue
            offline = prior.with_connection_state(
    ...
    def _discover_printers(self, *, observed_at) -> Iterable[DeviceRecord]:
        for driver in self.driver_registry.printer_drivers(available_only=False):
            ...
            try:
                ...
                logger.warning(
                    "Printer discovery failed for %s",
                    driver.metadata.key,
                    exc_info=True,
                )
                raise AgentError(
                    ...
                    status_code=503,
                    details={
                        "driver_key": driver.metadata.key,
...
def _device_event_payload(device: DeviceRecord) -> dict[str, object]:
    ...
    if device.kind is DeviceKind.PRINTER:
        payload["printer"] = {
            ...
            "preferred_transport": device.preferred_transport.value
            if device.preferred_transport is not None
            else None,
            "supported_transports": [
                transport.value for transport in device.supported_transports
            ],
