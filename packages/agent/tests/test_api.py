...
class StubRuntime:
    ...
    submit_print_job_error: Exception | None = None
    submit_command_error: Exception | None = None
    event_hub: EventHub = field(default_factory=EventHub)
    submitted_print_request: object | None = None
    submitted_command_request: object | None = None
    ...
    async def submit_print_job(self, request):
        if self.submit_print_job_error is not None:
            raise self.submit_print_job_error
        self.submitted_print_request = request
        return next(iter(self.jobs.values()))

    async def submit_command(self, request):
        if self.submit_command_error is not None:
            raise self.submit_command_error
        self.submitted_command_request = request
        return next(iter(self.jobs.values()))
...
class ApiShapeTests(unittest.TestCase):
    def test_system_status_reports_device_and_queue_summary(self) -> None:
        ...
        client = make_test_client(runtime)

        response = client.get("/system/status")

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["service"]["version"], "1.7.0a1")
    ...
    def test_submit_print_job_returns_queued_job_resource(self) -> None:
        ...
        self.assertEqual(runtime.submitted_print_request.target.printer_name, "Kitchen Printer")

    def test_submit_device_command_returns_queued_job_resource(self) -> None:
        runtime = make_stub_runtime(job_kind=JobKind.COMMAND, operation="cut_paper", command_kind="cut_paper")
        client = make_test_client(runtime)
        ...
        self.assertEqual(runtime.submitted_command_request.command.kind, "cut_paper")
    ...
    def test_validation_errors_use_unified_problem_details_shape(self) -> None:
        client = make_test_client(make_stub_runtime())
    ...
    def test_agent_errors_use_unified_problem_details_shape(self) -> None:
        runtime = make_stub_runtime()
        runtime.submit_print_job_error = AgentError(
            "DEVICE_NOT_FOUND",
...
def make_stub_runtime(
    *,
    ...
    job = JobRecord(
        ...
        request_payload={"content": {"kind": "text", "text": "Hello printer"}},
...
def make_test_client(runtime: StubRuntime) -> TestClient:
