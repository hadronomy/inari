...
fn environment_overrides_cover_every_nested_field() {
    ...
    assert_eq!(loaded.settings.server.shutdown_grace_period, Duration::from_secs(60));
    assert_eq!(loaded.settings.server.max_body_size_bytes, 4096);
    assert_eq!(loaded.settings.runtime.worker_threads.map(NonZeroUsize::get), Some(8));
    assert_eq!(loaded.settings.runtime.max_blocking_threads, 64);
    assert_eq!(loaded.settings.runtime.thread_stack_size_bytes, 4_194_304);
    assert_eq!(loaded.settings.runtime.event_interval, 99);
    assert_eq!(loaded.settings.runtime.global_queue_interval, 17);
    assert_eq!(loaded.settings.runtime.thread_keep_alive, Duration::from_secs(15));
    assert_eq!(loaded.settings.observability.service_name, "test-service");
    assert_eq!(loaded.settings.observability.filter, "info");
    assert_eq!(loaded.settings.observability.format, LogFormat::Json);
    assert!(!loaded.settings.observability.include_targets);
    assert!(loaded.settings.observability.include_thread_ids);
    assert!(!loaded.settings.observability.include_thread_names);
    ...
    assert!(loaded.settings.http.cors.allow_credentials);
    assert_eq!(loaded.settings.http.cors.max_age, Duration::from_secs(300));
    assert!(loaded.settings.http.zenoh_rest.enabled);
    assert!(loaded.settings.http.zenoh_rest.allow_admin_space);
    assert_eq!(loaded.settings.http.zenoh_rest.query_timeout, Duration::from_secs(11));
    assert_eq!(loaded.settings.http.zenoh_rest.sse_keep_alive, Duration::from_secs(20));
    assert_eq!(loaded.settings.http.zenoh_rest.sse_buffer, 128);
    assert!(loaded.settings.zenoh.enabled);
    assert_eq!(loaded.settings.zenoh.mode, ZenohMode::Router);
    assert!(loaded.settings.zenoh.admin_space.enabled);
    ...
    assert_eq!(loaded.settings.protocol.max_concurrent_requests, 2048);
}
